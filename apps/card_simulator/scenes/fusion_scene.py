...
fusion_scene.py
===============
Ecran de fusion — 3 phases :

  PHASE SELECT  : grille inventaire + 3 slots de depot + infos
  PHASE FUSING  : animation forge (barres de feu, flash)
  PHASE RESULT  : carte obtenue (succes) ou fumee (echec)

Interaction :
  - Clic sur une carte de la grille → place dans le prochain slot libre
  - Clic sur une carte dans un slot → la retire
  - Quand les 3 slots sont remplis de cartes de meme rareté → bouton Fusionner actif
"""
...
# ── Constantes ────────────────────────────────────────────────────────────────
# Les constantes de colonne sont calculées dynamiquement dans __init__
# en fonction de self.W pour s'adapter à toute résolution.
# Seules les constantes indépendantes de la largeur restent ici.
