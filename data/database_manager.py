...
Règle fondamentale : une seule connexion par opération publique.
Les fonctions internes (_internal_*) reçoivent un cursor existant
et ne créent jamais leur propre connexion → plus de deadlock.

Convention de nommage :
  fetch_*       → lecture seule, ouvre/ferme sa propre connexion
  db_*          → écriture, ouvre/ferme sa propre connexion
  _internal_*   → helpers sans connexion propre (cursor en paramètre)
  load_*        → charge un objet métier complet
"""
...
def load_player(player_id: int):
    ...
    try:
        ...
        # Nécessaire pour que les succès de type "first_rare" ne se
        # redéclenchent pas à chaque démarrage de session.
        cursor.execute("""
